    ...
    for line in deps_text.split("\n"):
        ...
        if "@" in line and "git+" in line:
            ...
            if package_name.startswith("adafruit-circuitpython-"):
                ...
                lib_name_camel = "".join(
                    p.upper() if len(p) <= 3 else p.capitalize() for p in lib_name_parts
                )
...
def add_submodule(frozen_dir: Path, dep: dict, dry_run: bool = False) -> bool:
    ...
    try:
        ...
        subprocess.run(
            cmd, check=True, cwd=frozen_dir.parent, capture_output=True, text=True
        )

        # Checkout specific version
        subprocess.run(
            ["git", "checkout", dep["version"]],
            check=True,
            cwd=submodule_path,
            capture_output=True,
            text=True,
...
def main():
    ...
    parser.add_argument(
        "--list", action="store_true", help="List dependencies without adding them"
    )
