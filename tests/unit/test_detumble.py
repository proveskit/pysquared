...
from pysquared.detumble import dot_product, magnetorquer_dipole, x_product
...
def test_dot_product():
    ...
    result = dot_product(mag_field_vector, mag_field_vector)
...
def test_x_product_negatives():
    ...
    actual_result = x_product(mag_field_vector, ang_vel_vector)
    assert pytest.approx(actual_result[0], 0.001) == expected_result[0]
    assert pytest.approx(actual_result[1], 0.001) == expected_result[1]
    assert pytest.approx(actual_result[2], 0.001) == expected_result[2]
...
def test_x_product_large_val():
    ...
    result = x_product(mag_field_vector, ang_vel_vector)
...
def test_magnetorquer_dipole():
    ...
    actual_result = magnetorquer_dipole(mag_field, ang_vel)
    assert pytest.approx(actual_result[0], 0.001) == expected_result[0]
    assert pytest.approx(actual_result[1], 0.001) == expected_result[1]
    assert pytest.approx(actual_result[2], 0.001) == expected_result[2]
...
def test_magnetorquer_dipole_zero_mag_field():
    ...
    with pytest.raises(ZeroDivisionError):
        magnetorquer_dipole(mag_field, ang_vel)
...
def test_magnetorquer_dipole_zero_ang_vel():
    ...
    mag_field = (30.0, -45.0, 60.0)
    ang_vel = (0.0, 0.0, 0.0)
    result = magnetorquer_dipole(mag_field, ang_vel)
    assert result == [0.0, 0.0, 0.0]
