...
def test_get_bus_voltage_failure(mock_i2c, mock_addr, mock_logger):
    ...
    type(power_monitor)._ina219 = PropertyMock(return_value=mock_ina219_instance)
    mock_ina219_instance.bus_voltage.side_effect = Exception("Simulated bus voltage failure")
...
def test_get_shunt_voltage_failure(mock_i2c, mock_addr, mock_logger):
    ...
    mock_ina219_instance = MagicMock(spec=INA219)
    type(power_monitor)._ina219 = PropertyMock(return_value=mock_ina219_instance)
    mock_ina219_instance.shunt_voltage.side_effect = Exception("Simulated shunt voltage failure")
...
def test_get_current_failure(mock_i2c, mock_addr, mock_logger):
    ...
    mock_ina219_instance = MagicMock(spec=INA219)
    type(power_monitor)._ina219 = PropertyMock(return_value=mock_ina219_instance)
    mock_ina219_instance.current.side_effect = Exception("Simulated current failure")
