# Notes:
# No password protection on message receive
...
# def _send_packet_with_retry(self, packet: bytes, seq_num: int) -> bool:
#     """Send a packet, if no acknowledgment is recieved, try again with a max number of retries"""
#     for attempt in range(self.max_retries):
#         self.radio.send(packet)

#         if self._wait_for_ack(seq_num):
...
# def send_durable(self, data: bytes) -> bool:
#     """Send data with packet retransmission on ack failures"""
#     packets: list[bytes] = self.packet_manager.pack_data(data)
#     total_packets: int = len(packets)
#     self.logger.info("Sending packets...", num_packets=total_packets)
...
#         if not self._send_packet_with_retry(packet, i):
#             self.logger.warning(
#                 "Failed to send data", failed_packet=i, num_packets=total_packets
#             )
#             return False
...
# I don't think handle_retransmit_request will work because nothing is storing the list of packets.
